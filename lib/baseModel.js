...
class BaseModel {
  ...
  get(key) {
    if (key in this.definition) {
      ...
      throw new Error(`Cannot get undefined property on model: ${key}`);
    }
  }
  ...
  set(key, value) {
    if (key in this.definition) {
      ...
      throw new Error(`Cannot set undefined property on model: ${key}`);
    }
  }
    ...
    return new Promise((resolve, reject) => {
      query.limit(opts.limit).find().then((data) => {
        ...
        for (var item of data) {
          output.push(BaseModel.createProxyFromModel(new this(item)));
        }
